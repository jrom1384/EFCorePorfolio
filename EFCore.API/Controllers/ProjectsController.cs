using AutoMapper;
using EFCore.API.Models;
using EFCore.Common;
using EFCore.DTO;
using EFCore.ServiceLayer;
using Microsoft.AspNetCore.Authentication.JwtBearer;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using System.Collections.Generic;
using System.Threading.Tasks;

namespace EFCore.API.Controllers
{
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
    [Route("api/[controller]")]
    [ApiController]
    public class ProjectsController : ControllerBase
    {
        private readonly IProjectService _service;
        private readonly IMapper _mapper;

        public ProjectsController(IProjectService service, IMapper mapper)
        {
            _service = service;
            _mapper = mapper;
        }

        // GET: api/Projects
        [HttpGet]
        public async Task<ActionResult<List<ProjectResponse>>> GetProject()
        {
            var result = await _service.GetListAsync();
            if (!result.IsSuccess)
            {
                return StatusCode(StatusCodes.Status500InternalServerError);
            }

            return Ok(_mapper.Map<List<ProjectResponse>>(result.Value));
        }

        // GET: api/Projects/Search
        [HttpGet("Search")]
        public async Task<ActionResult> Search(SearchRequest request)
        {
            var result = await _service.CreatePageListAsync(_mapper.Map<PageFilterDTO>(request));
            if (!result.IsSuccess)
            {
                return StatusCode(StatusCodes.Status500InternalServerError);
            }

            return Ok(new
            {
                result.Value.MatchCount,
                PageResult = _mapper.Map<List<ProjectResponse>>(result.Value)
            });
        }

        // GET: api/Projects/5
        [HttpGet("{id}")]
        public async Task<ActionResult<ProjectResponse>> GetProject(long id)
        {
            var result = await _service.FindByIdAsync(id);
            if (!result.IsSuccess)
            {
                return StatusCode(StatusCodes.Status500InternalServerError);
            }

            var response = _mapper.Map<ProjectResponse>(result.Value);
            if (response == null)
            {
                return NotFound();
            }

            return Ok(response);
        }

        // PUT: api/Projects/5
        [HttpPut("{id}")]
        public async Task<ActionResult<ProjectResponse>> PutProject(long id, ProjectRequest request)
        {
            if (id != request.Id)
            {
                return BadRequest();
            }

            var result = await _service.UpdateAsync(_mapper.Map<ProjectDTO>(request));
            if (!result.IsSuccess)
            {
                if (result.ErrorType == ErrorType.Defined)
                {
                    if (result.ErrorMessage.Contains(ErrorMessages.RecordAlreadyExists))
                    {
                        return StatusCode(StatusCodes.Status403Forbidden);
                    }
                    else
                    {
                        return BadRequest();
                    }
                }
                //else if (result.ErrorType == ErrorType.DBUpdateConcurrency && !await ProjectExists(id))
                //{
                //    return NotFound();
                //}
                else
                {
                    return StatusCode(StatusCodes.Status500InternalServerError);
                }
            }

            return Ok(_mapper.Map<ProjectResponse>(result.Value));
        }

        // POST: api/Projects
        [HttpPost]
        public async Task<ActionResult<ProjectResponse>> PostProject(NewProjectRequest request)
        {
            var result = await _service.AddAsync(_mapper.Map<ProjectDTO>(request));
            if (!result.IsSuccess)
            {
                if (result.ErrorType == ErrorType.Defined)
                {
                    if (result.ErrorMessage.Contains(ErrorMessages.RecordAlreadyExists))
                    {
                        return StatusCode(StatusCodes.Status403Forbidden);
                    }
                    else
                    {
                        return BadRequest();
                    }
                }
                else
                {
                    return StatusCode(StatusCodes.Status500InternalServerError);
                }
            }

            var response = _mapper.Map<ProjectResponse>(result.Value);
            return CreatedAtAction(nameof(GetProject), new { id = response.Id }, response);
        }

        // DELETE: api/Projects/5
        [HttpDelete("{id}")]
        public async Task<ActionResult<ProjectResponse>> DeleteProject(long id)
        {
            var result = await _service.DeleteByIdAsync(id);
            if (!result.IsSuccess)
            {
                return StatusCode(StatusCodes.Status500InternalServerError);
            }

            var response = _mapper.Map<ProjectResponse>(result.Value);
            if (response == null)
            {
                return NotFound();
            }

            return Ok(response);
        }

        private async Task<bool> ProjectExists(long id)
        {
            var result =  await _service.AnyAsync(e => e.Id == id);
            if (!result.IsSuccess)
            {
                return false;
            }

            return result.Value;
        }
    }
}
